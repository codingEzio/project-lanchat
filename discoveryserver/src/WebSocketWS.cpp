...
void WebSocketWS::StartServerWS(uint32_t nPort, string sEndURL) {
  ...
  echo.on_message = [this, &echo](shared_ptr<WsServer::Connection> connection,
  ...
  echo.on_close = [this](shared_ptr<WsServer::Connection> connection,
                         int status, const string & /*reason*/) {
    OnDisconnected(connection);
  };

  echo.on_error = [this](shared_ptr<WsServer::Connection> connection,
...
void WebSocketWS::SendMessage(shared_ptr<WsServer::Connection> hConnection,
                              string sMessage) {
  hConnection->send(sMessage);
}

void WebSocketWS::timer_start(std::function<void(void)> func,
